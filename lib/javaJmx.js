...
function JavaJmx(serviceOrHost, port, protocol, urlPath) {
  ...
  function subscribeTo(obj) {
    ...
    obj.on("error", function(err) {
     ...
     // } else if (/java\.net\.SocketException: Connection reset/.test(err)) {
     //   console.log('InvocationTargetException')
     //   self.mbeanServerConnection.close();
     //self.emit("disconnect");
      } else if(err == `MBeanServerConnection: Premature disconnect has occurred.`){
        console.log(`Произошло преждевременное отключение (${err})`)
      } else {
        self.emit("error", err);
      }
    });
